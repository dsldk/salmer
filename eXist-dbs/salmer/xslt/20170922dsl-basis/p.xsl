<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs tei" version="2.0">
    ...
    <xsl:template match="tei:p">
        ...
        <!--
        <xsl:choose>-->
            <!--<xsl:when test="tei:note | tei:lg//tei:note | tei:app | tei:lg//tei:app">-->
            <!--<xsl:when test="//tei:note">
                <div class="text-and-note">
                    <ul class="sidenotes">
                        <li>
                            <xsl:apply-templates/>-->
                            <!--<xsl:apply-templates
                                select="tei:note | tei:lg//tei:note | tei:lg//tei:app"
                    ...
                    <p>
                        <span class="p-number">
                            <xsl:attribute name="id">
                                <xsl:value-of select="$para-number"/> </xsl:attribute>
                                ¶<xsl:value-of select="$para-number"/>
                        </span>
                        <xsl:apply-templates/>
                    </p>
